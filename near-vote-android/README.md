...
- 두 기기에서 같은 버튼으로 광고와 탐색을 동시에 시작하는 연결 진단
...
3. 앱 권한 요청을 모두 허용합니다.
4. 두 기기 모두 `개발자 진단`으로 이동합니다.
5. 두 기기 모두 `주변 연결 시작`을 누릅니다.
6. `연결된 기기 1대`가 보이면 한쪽에서 `테스트 메시지 보내기`를 누릅니다.
7. 반대쪽 로그에 메시지 수신 기록이 보이면 Nearby 통신이 확인된 것입니다.

## 투표 게시와 참여 테스트

1. 두 기기에서 먼저 `개발자 진단` > `주변 연결 시작`으로 연결합니다.
2. 제안자 기기에서 `설문 만들기` > `주변에 게시하기`를 누릅니다.
3. 참여자 기기에 `투표 참여` 화면이 뜨면 선택지를 누릅니다.
4. 제안자 기기의 `게시한 투표` 화면에서 집계가 갱신되는지 확인합니다.
5. 설문 화면이 늦게 뜨면 제안자 기기에서 `참여 요청 다시 보내기`를 누릅니다.
